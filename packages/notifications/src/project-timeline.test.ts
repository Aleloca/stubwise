...
import { buildProjectTimeline } from "./project-timeline.js";
 ...
 *
 * Il test vive QUI e non in `apps/server` perché qui vive la funzione: la
 * fusione serve alla rotta della roadmap E al brief settimanale del worker
 * (vedi il commento in testa a `project-timeline.ts`). Nel server restano i
 * test di ciò che è HTTP — finestra dalla querystring, ACL, `listProjectReviews`.
 */
let testDb: TestDb;
let db: Db;
