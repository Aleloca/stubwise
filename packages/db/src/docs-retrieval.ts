 ...
 *
 * Vive in `@stubwise/db` (non nel server) perché serve sia al server (ricerca,
 * chat RAG) sia al worker (intake del backlog di discovery + similarity search):
 * entrambi la importano da qui senza duplicarla. Per non accoppiare il package db
 * a `@stubwise/embeddings` (né a Fastify), l'embedder è un tipo STRUTTURALE locale
 * ({@link EmbeddingProvider}) e il logger un sottoinsieme strutturale di pino:
 * il client reale e `request.log` li soddisfano senza import.
 */

import { and, eq, inArray, isNull, or, sql } from "drizzle-orm";
import { docChunks, docPages, repositories } from "./schema.js";
import type { Db } from "./client.js";

/**
 * Contratto minimo dell'embedder usato dal retrieval: `embed(testi)` → un vettore
 * per testo. Tipo STRUTTURALE locale (nessun import da `@stubwise/embeddings`) così
 * il package db non dipende dal package embeddings; il client reale
 * (`EmbeddingClient`) e i fake dei test lo soddisfano per forma.
 */
