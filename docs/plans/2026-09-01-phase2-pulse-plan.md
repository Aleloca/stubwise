...
5. **Rollback di immagine, non solo di env.** Spegnere la feature (`PULSE_POLL_MINUTES=0` o i toggle) è innocuo; **tornare all'immagine server precedente non lo è** finché in `notifications` esiste una riga `project.pulse`, anche solo **gestita**: il kind non esiste nel binario vecchio, `inboxPageSchema` fa fallire la serializzazione e cade tutta `/api/inbox` con un 500. Annotato in `CLAUDE.md` § Deploy; la via d'uscita è eliminare quelle righe prima di scendere di immagine.
6. **Da osservare ai primi feedback: snooze contro cadenza corta.** Con `pulseEveryDays = 1` uno snooze può essere di fatto neutralizzato — il pulse del giorno dopo chiude la copia rinviata prima che la posticipazione scada. Coi default (cadenza 3) le due scale combaciano e il caso non si presenta. Se qualcuno mette la cadenza a 1 e si lamenta dello snooze, la leva è la cadenza, non lo snooze.

Dalla review finale d'insieme, da valutare a parte (nulla implementato):

7. **Lingua dei contenuti del payload** — vedi il punto 1: la review conferma che è la cosa da sistemare per prima il giorno in cui l'istanza serve utenti non italiani.
8. **«Qualcun altro» impreciso quando a chiudere è il poller.** Quando è il sistema a chiudere una copia (non un utente), il messaggio mostrato attribuisce comunque la chiusura a una persona generica. `handled_by_user_id` è già nullo in quel caso: il testo può distinguerlo.
9. **Il pulse sopravvive alla ripresa del lavoro.** Se dopo il pulse il progetto riparte da solo (qualcuno apre un ticket, un job va in volo), le copie aperte restano lì a proporre lavoro nuovo: la ragione per cui erano state mandate non c'è più. Da decidere se scadono.
10. **Un click su una proposta stale brucia l'intero pulse.** Se la voce scelta non è più convertibile, l'errore chiude la partita per tutte e tre le proposte, comprese quelle ancora valide.
11. **Archiviare un pulse non insegna niente al ranking.** Chi scarta sistematicamente le stesse proposte non ha modo di farlo sapere: il ranking successivo le ripropone identiche.
