 ...
 *
 * FORMA DEL graph.json (node-link NetworkX, graphify 0.9.28): `nodes` con
 * `id`/`label`/`source_file`/`community`/`community_name`, archi in `links` con
 * `source`/`target` (id dei nodi). Il parse è TOLLERANTE: ogni nodo o arco che
 * non ha la forma attesa viene ignorato, non fa fallire il calcolo.
 *
 * NIENTE CACHE (YAGNI): le review sono rare e un grafo da qualche MB si parsa
 * in meno di un secondo. Il file letto è quello del VOLUME (sempre fresco,
 * rigenerato sul push), non l'eventuale copia committata nel repo.
 *
...
/** Nodo del grafo ridotto a ciò che serve qui (già validato). */
interface GraphNode {
  id: string;
  label: string;
  sourceFile: string;
  ...
  const community =
    typeof name === "string" && name.length > 0
      ...
      : typeof number === "number" || typeof number === "string"
        ? `Community ${number}`
        : "";
  return { id, label, sourceFile: normalizePath(sourceFile), community };
}

/** Grado (archi incidenti) per id di nodo. Archi malformati ignorati. */
function degreesByNodeId(links: unknown[]): Map<string, number> {
 ...
 */
function godNodeThreshold(nodes: GraphNode[], degrees: Map<string, number>): number {
...
export async function computeBlastRadius(
  ...
  try {
    ...
    const parsed: unknown = JSON.parse(await readFile(input.graphJsonPath, "utf8"));
